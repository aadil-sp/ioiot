...
import { Cpu, LogOut, LayoutDashboard, Shield, Sun, Moon, User, Menu, X, Bell, Code2 } from 'lucide-react';
...
function NavContent({ auth, setAuth, dark, toggle, mobile, onClose }) {
  ...
  return (
    <>
      ...
      {/* Web Editor — always visible, no login required */}
      <Link to="/web-editor" onClick={onClose}
        className={`flex items-center gap-2 text-sm px-4 py-2 rounded-xl border transition-all ${dark ? 'bg-[#1a1a2e] border-purple-500/30 text-purple-400 hover:bg-purple-500/10 hover:border-purple-500 shadow-[0_0_12px_rgba(168,85,247,0.15)]' : 'bg-purple-50 border-purple-200 text-purple-700 hover:bg-purple-100'}`}>
        <Code2 className="w-4 h-4" />
        <span className="hidden sm:inline font-bold">Web Editor</span>
      </Link>
      ...
      {token && (
        <>
          ...
          {/* Admins also get a "My Devices" link to create/manage their own devices */}
          {role === 'admin' && (
    ...
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-300 ${dark ? 'bg-[#12121a] text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      <header className={`px-4 sm:px-6 py-4 border-b flex justify-between items-center sticky top-0 z-50 backdrop-blur-2xl transition-colors duration-300 ${dark ? 'border-orange-500/40 bg-[#12121a]/80 shadow-[0_4px_30px_rgba(0,0,0,0.4)]' : 'border-orange-300/40 bg-white/90'}`}>
        ...
        <Link to="/" className="flex items-center gap-2 sm:gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-black tracking-wider bg-gradient-to-r from-orange-400 to-red-600 bg-clip-text text-transparent">
            IoIoT
          </h1>
      ...
      <main className="flex-1 relative overflow-x-hidden">
        ...
        <div className="relative z-10">
          <Routes>
            ...
            {/* Web Editor — public, no login required */}
